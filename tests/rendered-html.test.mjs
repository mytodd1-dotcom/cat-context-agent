...
test("server-renders the CAT Context Agent hackathon shell", async () => {
  ...
  assert.match(html, /2-minute judge card/);
  assert.match(html, /npm run judge:quick/);
  assert.match(html, /judge-quick-card\.md/);
  assert.match(html, /Judge rubric matrix/);
  assert.match(html, /npm run judge:rubric/);
  assert.match(html, /judge-rubric-matrix\.md/);
  ...
  assert.match(html, /npm run judge:faq/);
  assert.match(html, /judge-faq\.md/);
  ...
  assert.match(html, /Submission honesty audit/);
  assert.match(html, /npm run submission:honesty/);
  assert.match(html, /submission-honesty-audit\.md/);
...
test("keeps the project shell responsive and repo-ready", async () => {
  ...
  assert.match(page, /judge-faq\.md/);
  ...
  assert.match(page, /submission-honesty-audit\.md/);
  ...
  assert.match(packageJson, /"datahub:roundtrip": "node scripts\/datahub-live-roundtrip\.mjs"/);
  ...
  assert.match(packageJson, /"submission:honesty": "node scripts\/submission-honesty-audit\.mjs"/);
  ...
  assert.match(packageJson, /"judge:faq": "node scripts\/judge-faq\.mjs"/);
  assert.match(packageJson, /"judge:quick": "node scripts\/judge-quick-card\.mjs"/);
  assert.match(packageJson, /"judge:rubric": "node scripts\/judge-rubric-matrix\.mjs"/);
  ...
  assert.match(packageJson, /"ci:local": "npm ci --dry-run && npm run context:contracts && npm run datahub:payload && npm run datahub:runbook && npm run datahub:doctor && npm run datahub:roundtrip && npm run datahub:checklist && npm run datahub:audit && npm run datahub:mcp && npm run mcp:smoke && npm run devpost:copy && npm run submission:honesty && npm run decision:trace && npm run lineage:map && npm run policy:matrix && npm run submission:verify && npm run judge:walkthrough && npm run judge:faq && npm run judge:quick && npm run judge:rubric && npm run artifacts:validate && npm run judge:brief && npm run submission:index && npm run demo:guide && npm test"/);
  ...
  assert.match(readme, /datahub-live-roundtrip\.md/);
  assert.match(readme, /npm run datahub:roundtrip/);
  assert.match(readme, /entitiesV2/);
  ...
  assert.match(readme, /submission-honesty-audit\.md/);
  assert.match(readme, /npm run submission:honesty/);
  ...
  assert.match(readme, /judge-faq\.md/);
  assert.match(readme, /npm run judge:faq/);
  assert.match(readme, /judge-quick-card\.md/);
  assert.match(readme, /npm run judge:quick/);
  assert.match(readme, /judge-rubric-matrix\.md/);
  assert.match(readme, /npm run judge:rubric/);
  ...
  assert.match(judgeStart, /judge-quick-card\.md/);
  assert.match(judgeStart, /judge-rubric-matrix\.md/);
  ...
  assert.match(judgeStart, /submission-honesty-audit\.md/);
  ...
  assert.match(judgeNotes, /judge FAQ/);
  assert.match(judgeNotes, /npm run judge:faq/);
  assert.match(judgeNotes, /2-minute judge card/);
  assert.match(judgeNotes, /npm run judge:quick/);
  assert.match(judgeNotes, /judge rubric matrix/);
  assert.match(judgeNotes, /npm run judge:rubric/);
  ...
  assert.match(judgeNotes, /submission honesty audit/);
  assert.match(judgeNotes, /npm run submission:honesty/);
  ...
  assert.match(ciWorkflow, /npm run datahub:roundtrip/);
  ...
  assert.match(ciWorkflow, /npm run submission:honesty/);
  assert.match(ciWorkflow, /npm run judge:walkthrough/);
  assert.match(ciWorkflow, /npm run judge:faq/);
  assert.match(ciWorkflow, /npm run judge:quick/);
  assert.match(ciWorkflow, /npm run judge:rubric/);
...
test("builds a judge evidence pack from generated artifacts", async () => {
  ...
  assert.ok(pack.judge_commands.includes("npm run datahub:roundtrip"));
  ...
  assert.ok(pack.judge_commands.includes("npm run judge:faq"));
  assert.ok(pack.judge_commands.includes("npm run judge:quick"));
  assert.ok(pack.judge_commands.includes("npm run judge:rubric"));
  ...
  assert.ok(pack.artifacts_to_inspect.includes("hackathon-assets/datahub-live-roundtrip.md"));
  ...
  assert.ok(pack.artifacts_to_inspect.includes("hackathon-assets/judge-faq.md"));
  assert.ok(pack.artifacts_to_inspect.includes("hackathon-assets/judge-quick-card.md"));
  assert.ok(pack.artifacts_to_inspect.includes("hackathon-assets/judge-rubric-matrix.md"));
  ...
  assert.match(packMarkdown, /judge-faq\.md/);
  assert.match(packMarkdown, /judge-quick-card\.md/);
  assert.match(packMarkdown, /judge-rubric-matrix\.md/);
...
test("verifies the complete submission evidence chain", async () => {
  ...
  assert.match(reportMarkdown, /✅ \*\*submission honesty audit\*\*/);
...
test("validates generated evidence artifacts", async () => {
  ...
  await execFileAsync("node", ["scripts/datahub-live-roundtrip.mjs"], {
    cwd: new URL("..", import.meta.url),
  });
  ...
  await execFileAsync("node", ["scripts/judge-faq.mjs"], {
    cwd: new URL("..", import.meta.url),
  });
  await execFileAsync("node", ["scripts/judge-quick-card.mjs"], {
    cwd: new URL("..", import.meta.url),
  });
  await execFileAsync("node", ["scripts/judge-rubric-matrix.mjs"], {
    cwd: new URL("..", import.meta.url),
  });
  await execFileAsync("node", ["scripts/submission-honesty-audit.mjs"], {
    cwd: new URL("..", import.meta.url),
  });
  ...
  assert.ok(report.validated_files.includes("hackathon-assets/datahub-live-roundtrip.json"));
  ...
  assert.ok(report.validated_files.includes("hackathon-assets/submission-honesty-audit.json"));
  ...
  assert.ok(report.validated_files.includes("hackathon-assets/judge-faq.json"));
  assert.ok(report.validated_files.includes("hackathon-assets/judge-quick-card.json"));
  assert.ok(report.validated_files.includes("hackathon-assets/judge-rubric-matrix.json"));
  ...
  assert.match(markdown, /✅ \*\*DataHub live roundtrip harness\*\*/);
  ...
  assert.match(markdown, /✅ \*\*submission honesty audit\*\*/);
  ...
  assert.match(markdown, /✅ \*\*judge FAQ\*\*/);
  assert.match(markdown, /✅ \*\*judge quick card\*\*/);
  assert.match(markdown, /✅ \*\*judge rubric matrix\*\*/);
});
...
test("reproduces the judge evidence chain with one command", async () => {
  ...
  assert.equal(receipt.checks.length, 20);
  assert.equal(receipt.summary.total_requests, 3);
  assert.equal(receipt.summary.artifact_validation_checks, 21);
  assert.equal(receipt.summary.live_roundtrip_mode, "dry-run");
  ...
  assert.ok(receipt.reports.includes("hackathon-assets/datahub-live-roundtrip.md"));
  ...
  assert.ok(receipt.reports.includes("hackathon-assets/submission-honesty-audit.md"));
  ...
  assert.ok(receipt.reports.includes("hackathon-assets/judge-faq.md"));
  assert.ok(receipt.reports.includes("hackathon-assets/judge-quick-card.md"));
  assert.ok(receipt.reports.includes("hackathon-assets/judge-rubric-matrix.md"));
...
test("generates a hard-question judge FAQ", async () => {
  const { stdout } = await execFileAsync("node", ["scripts/judge-faq.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  assert.match(stdout, /cat-judge-faq-v0/);
  assert.match(stdout, /"status": "ready"/);
  assert.match(stdout, /judge-faq\.md/);

  const [faq, markdown] = await Promise.all([
    readFile(new URL("../hackathon-assets/judge-faq.json", import.meta.url), "utf8").then(JSON.parse),
    readFile(new URL("../hackathon-assets/judge-faq.md", import.meta.url), "utf8"),
  ]);

  assert.equal(faq.protocol, "cat-judge-faq-v0");
  assert.equal(faq.status, "ready");
  assert.equal(faq.questions.length, 5);
  assert.equal(faq.quick_facts.external_side_effects, "none");
  assert.equal(faq.quick_facts.local_datahub_required_for_judging, false);
  assert.ok(faq.quick_facts.blocked_actions.includes("scrape_contact_details"));
  assert.ok(faq.questions.some((item) => item.question.includes("live DataHub")));
  assert.ok(faq.questions.some((item) => item.command === "npm run submission:honesty"));
  assert.match(markdown, /Judge FAQ/);
  assert.match(markdown, /Questions judges may ask/);
  assert.match(markdown, /Is this connected to live DataHub right now/);
  assert.match(markdown, /What prevents unsafe automation/);
});

test("generates a 2-minute judge quick card", async () => {
  const { stdout } = await execFileAsync("node", ["scripts/judge-quick-card.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  assert.match(stdout, /cat-judge-quick-card-v0/);
  assert.match(stdout, /"status": "ready"/);
  assert.match(stdout, /judge-quick-card\.md/);
  assert.match(stdout, /npm run evidence:reproduce/);

  const [card, markdown] = await Promise.all([
    readFile(new URL("../hackathon-assets/judge-quick-card.json", import.meta.url), "utf8").then(JSON.parse),
    readFile(new URL("../hackathon-assets/judge-quick-card.md", import.meta.url), "utf8"),
  ]);

  assert.equal(card.protocol, "cat-judge-quick-card-v0");
  assert.equal(card.status, "ready");
  assert.equal(card.time_budget_minutes, 2);
  assert.equal(card.open_first.length, 4);
  assert.equal(card.fastest_local_command, "npm run evidence:reproduce");
  assert.equal(card.full_validation_command, "npm run ci:local");
  assert.equal(card.datahub_evidence.local_datahub_required_for_judging, false);
  assert.equal(card.safety_boundary.external_side_effects, "none");
  assert.equal(card.safety_boundary.secrets_required, false);
  assert.ok(card.safety_boundary.blocked_actions.includes("scrape_contact_details"));
  assert.ok(card.claim_snapshot.some((item) => item.files.includes("hackathon-assets/safety-policy-matrix.md")));
  assert.match(markdown, /2-Minute Judge Card/);
  assert.match(markdown, /Fastest proof/);
  assert.match(markdown, /Claim snapshot/);
  assert.match(markdown, /DataHub evidence/);
});

test("generates a judge rubric matrix", async () => {
  const { stdout } = await execFileAsync("node", ["scripts/judge-rubric-matrix.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  assert.match(stdout, /cat-judge-rubric-matrix-v0/);
  assert.match(stdout, /"status": "ready"/);
  assert.match(stdout, /judge-rubric-matrix\.md/);
  assert.match(stdout, /npm run judge:quick/);

  const [matrix, markdown] = await Promise.all([
    readFile(new URL("../hackathon-assets/judge-rubric-matrix.json", import.meta.url), "utf8").then(JSON.parse),
    readFile(new URL("../hackathon-assets/judge-rubric-matrix.md", import.meta.url), "utf8"),
  ]);

  assert.equal(matrix.protocol, "cat-judge-rubric-matrix-v0");
  assert.equal(matrix.status, "ready");
  assert.equal(matrix.criteria.length, 6);
  assert.equal(matrix.fastest_scoring_command, "npm run judge:quick");
  assert.equal(matrix.full_validation_command, "npm run ci:local");
  assert.equal(matrix.signals.external_side_effects, "none");
  assert.equal(matrix.signals.honesty_status, "passed");
  assert.ok(matrix.source.url.includes("datahub.devpost.com"));
  assert.ok(matrix.criteria.some((item) => item.criterion === "Use of DataHub"));
  assert.ok(matrix.criteria.some((item) => item.criterion === "Technical Execution"));
  assert.ok(matrix.criteria.some((item) => item.criterion === "Bonus: Meaningful Open-Source Contribution"));
  assert.match(markdown, /Judge Rubric Matrix/);
  assert.match(markdown, /Rubric alignment/);
  assert.match(markdown, /Use of DataHub/);
  assert.match(markdown, /Submission Quality/);
});

test("generates a submission honesty audit", async () => {
  await execFileAsync("node", ["scripts/devpost-submission-copy.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  const { stdout } = await execFileAsync("node", ["scripts/submission-honesty-audit.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  assert.match(stdout, /cat-submission-honesty-audit-v0/);
  assert.match(stdout, /"status": "passed"/);
  assert.match(stdout, /submission-honesty-audit\.md/);

  const [audit, markdown] = await Promise.all([
    readFile(new URL("../hackathon-assets/submission-honesty-audit.json", import.meta.url), "utf8").then(JSON.parse),
    readFile(new URL("../hackathon-assets/submission-honesty-audit.md", import.meta.url), "utf8"),
  ]);

  assert.equal(audit.protocol, "cat-submission-honesty-audit-v0");
  assert.equal(audit.status, "passed");
  assert.equal(audit.audits.length, 5);
  assert.ok(audit.audits.every((item) => item.ok));
  assert.ok(audit.disallowed_overclaims.includes("sends customer outreach automatically"));
  assert.ok(audit.explicit_boundaries.some((boundary) => boundary.includes("runnable without DataHub credentials")));
  assert.match(markdown, /Submission Honesty Audit/);
  assert.match(markdown, /no-overclaim language/);
  assert.match(markdown, /external_side_effects as none/);
});
...
test("generates a judge scoring brief from reproduced evidence", async () => {
  ...
  assert.ok(brief.claims.some((claim) => claim.files.includes("hackathon-assets/judge-faq.md")));
  assert.ok(brief.claims.some((claim) => claim.files.includes("hackathon-assets/judge-quick-card.md")));
  assert.ok(brief.claims.some((claim) => claim.files.includes("hackathon-assets/judge-rubric-matrix.md")));
  assert.ok(brief.claims.some((claim) => claim.files.includes("hackathon-assets/submission-honesty-audit.md")));
...
test("generates a judge-first submission index", async () => {
  ...
  assert.equal(index.suggested_review_order.length, 18);
  ...
  assert.ok(index.proof_commands.includes("npm run judge:faq"));
  assert.ok(index.proof_commands.includes("npm run judge:quick"));
  assert.ok(index.proof_commands.includes("npm run judge:rubric"));
  ...
  assert.ok(index.proof_commands.includes("npm run submission:honesty"));
  ...
  assert.match(markdown, /judge-faq\.md/);
  assert.match(markdown, /judge-quick-card\.md/);
  assert.match(markdown, /judge-rubric-matrix\.md/);
  ...
  assert.match(markdown, /submission-honesty-audit\.md/);
...
test("generates a local DataHub roundtrip harness without posting metadata", async () => {
  const { stdout } = await execFileAsync("node", ["scripts/datahub-live-roundtrip.mjs"], {
    cwd: new URL("..", import.meta.url),
  });

  assert.match(stdout, /cat-datahub-live-roundtrip-v0/);
  assert.match(stdout, /ready_for_local_roundtrip/);
  assert.match(stdout, /datahub-live-roundtrip\.md/);

  const [roundtrip, markdown] = await Promise.all([
    readFile(new URL("../hackathon-assets/datahub-live-roundtrip.json", import.meta.url), "utf8").then(JSON.parse),
    readFile(new URL("../hackathon-assets/datahub-live-roundtrip.md", import.meta.url), "utf8"),
  ]);

  assert.equal(roundtrip.protocol, "cat-datahub-live-roundtrip-v0");
  assert.equal(roundtrip.status, "ready_for_local_roundtrip");
  assert.equal(roundtrip.mode, "dry-run");
  assert.equal(roundtrip.mutations_performed, false);
  assert.equal(roundtrip.safety.remote_or_production_posting_allowed, false);
  assert.equal(roundtrip.write_plan.action, "ingestProposal");
  assert.deepEqual(roundtrip.write_plan.aspects, ["datasetProperties", "schemaMetadata", "ownership", "glossaryTerms"]);
  assert.match(roundtrip.readback_plan.entity_endpoint, /\/entitiesV2\//);
  assert.equal(roundtrip.readback_plan.aspect_endpoints.length, 4);
  assert.ok(roundtrip.checks.every((check) => check.ok));
  assert.equal(roundtrip.optional_live_command, "DATAHUB_GMS_URL=http://localhost:8080 npm run datahub:roundtrip -- --post");
  assert.match(markdown, /DataHub Live Roundtrip Harness/);
  assert.match(markdown, /No metadata was posted/);
  assert.match(markdown, /entitiesV2/);
});
